...
typedef pair<int, char> ExchangeType;
bool findPath(const string& src, const string& dst, const set<string>& dictionary, vector<string>& steps)
{
	if (src == dst)
		return true;
	...
	{
		if (dictionary.end() == dictionary.find(t))
			continue;

		if (findPath(t, dst, dictionary, steps))
		{
			steps.push_back(t);
			return true;
		}
...
void JamesChen25()
{
	...
	vector<string> steps;
	const auto found = findPath(source, target, dic, steps);
	if (found)
		cout << "Found" << endl;
	else
		cout << "NOT found" << endl;
